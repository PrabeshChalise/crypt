...
const AdminDepositApproval = () => {
  ...
  useEffect(() => {
    if (userId) {
      fetchDepositsForUser(userId); // Fetch deposits for a specific user if userId is present
    } else {
      fetchPendingDeposits(); // Fetch all pending deposits if no userId is present
    }
  }, [userId]);

  const fetchPendingDeposits = async () => {
    try {
      const response = await axios.get("https://trcnxf.com/api/deposits");
  ...
  const fetchDepositsForUser = async (userId) => {
    try {
      const response = await axios.get(
        `https://trcnxf.com/api/deposits/user/${userId}`
      );
      setPendingDeposits(response.data);
  ...
  const handleApprove = async (id, updatedAmount) => {
    ...
    try {
      await axios.post(`https://trcnxf.com/api/deposits/${id}/approve`, {
  ...
  const handleDecline = async (id) => {
    try {
      await axios.delete(`https://trcnxf.com/api/deposits/${id}`);
  ...
  const handleConfirmAction = async () => {
    ...
    try {
      await axios.post(
        `https://trcnxf.com/api/deposits/${currentDeposit._id}/complete`
      );
