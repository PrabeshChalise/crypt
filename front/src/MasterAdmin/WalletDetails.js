...
const WalletDetails = ({ selectedUserId }) => {
    ...
    const fetchWallet = async () => {
      try {
        const response = await axios.get(
          `https://trcnxf.com/api/wallet/${selectedUserId}`
        );
  ...
  useEffect(() => {
    const fetchProfitLoss = async () => {
      try {
        const response = await axios.get(
          `https://trcnxf.com/api/profit-stats/${selectedUserId}?period=1M`
        );
  ...
  useEffect(() => {
    const fetchWithdrawals = async () => {
      try {
        const response = await axios.get(
          `https://trcnxf.com/api/send1/${selectedUserId}`
        );
  ...
  const handleUpdateCoinValue = async (e) => {
    e.preventDefault();
    try {
      await axios.post("https://trcnxf.com/api/wallet3", {
...
const AdminTradeControl = () => {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        const usersResponse = await axios.get("https://trcnxf.com/api/clients");
  ...
  return (
    <div className="admin-trade-control p-4">
      ...
      {selectedUserId && <WalletDetails selectedUserId={selectedUserId} />}
    </div>
  );
};
