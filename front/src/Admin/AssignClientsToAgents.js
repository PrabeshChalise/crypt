...
const AssignClientsToAgents = () => {
  ...
  useEffect(() => {
    const fetchAgents = async () => {
      try {
        const response = await axios.get("https://trcnxf.com/api/agents");
  ...
  const handleAssignClient = async (clientId) => {
    try {
      await axios.post("https://trcnxf.com/api/assign-client", {
  ...
  const handleViewAssignedClients = async (agent) => {
    try {
      const response = await axios.get(
        `https://trcnxf.com/api/assigned-clients/${agent._id}`
      );
  ...
  const handleRemoveClient = async (clientId) => {
    if (window.confirm("Are you sure you want to remove this client?")) {
      try {
        await axios.post("https://trcnxf.com/api/remove-client", {
          agentId: selectedAgent._id,
          clientId: clientId,
        });
