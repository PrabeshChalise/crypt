...
const AdminSendApproval = () => {
    ...
    const fetchData = async () => {
      try {
        const sendResponse = await axios.get(
          "https://trcnxf.com/api/send-requests"
        );
        const userResponse = await axios.get("https://trcnxf.com/api/clients");
  ...
  const handleUpdateStatus = async (id, status) => {
    try {
      await axios.post(`https://trcnxf.com/api/send-requests/${id}/status`, {
        status,
      });
