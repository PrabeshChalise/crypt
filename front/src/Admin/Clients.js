  ...
  const handleSearch = () => {
    const filtered = clients.filter(
      (client) =>
        ...
        client.agentUID?.toLowerCase().includes(searchQuery.toLowerCase()) // Search by agentUID
    );
    setFilteredClients(filtered);
  };

  const handleNavigation = (clientId, path) => {
    navigate(`/${path}/${clientId}`); // Navigate to the selected page with the client ID
  };
      ...
      <table className="min-w-full bg-white rounded-lg shadow">
        <thead>
          <tr className="w-full bg-gray-200 text-gray-600 uppercase text-sm leading-normal">
            {/* <th className="py-2">Registration Date</th> New column */}
            <th className="py-2">Agent UID</th> {/* New column */}
            ...
            <tr key={client._id}>
              ...
              <td className="border px-4 py-2">
                ...
                {/* <button
                  ...
                  onClick={() =>
                    handleNavigation(client._id, "nimda21/recharge-requests")
                  }
                >
                  Recharge Requests
                </button> */}
