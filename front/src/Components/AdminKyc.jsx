...
const AdminKyc = () => {
  ...
  useEffect(() => {
    const fetchKycRequests = async () => {
      try {
        const response = await axios.get("https://trcnxf.com/api/kyc");
  ...
  const handleApprove = async (id) => {
    try {
      await axios.post(`https://trcnxf.com/api/kyc/${id}/approve`);
  ...
  const handleReject = async (id) => {
    try {
      await axios.post(`https://trcnxf.com/api/kyc/${id}/reject`);
      setKycRequests(kycRequests.filter((request) => request._id !== id));
    } catch (error) {
        ...
        <tbody>
          {kycRequests.map((request) => (
            <tr key={request._id}>
              ...
              <td style={styles.td}>
                <img
                  src={`https://trcnxf.com/${request.identityProof}`}
                  alt="Identity Proof"
                  style={styles.image}
                />
              </td>
              <td style={styles.td}>
                <img
                  src={`https://trcnxf.com/${request.photo}`}
                  alt="Photo"
                  style={styles.image}
                />
              </td>
