...
app.post("/api/convert", async (req, res) => {
  ...
  try {
    ...
    // Deduct amount from the source balance (handling USDT as "tether")
    if (
...
app.post("/api/withdraw", async (req, res) => {
  ...
  try {
    ...
    wallet.balances.set("tether", wallet.balances.get("tether") + usdAmount);
    await wallet.save();
...
app.post("/api/send", async (req, res) => {
  ...
  try {
    ...
    // Deduct amount from both usd and tether if symbol is tether or usd
    if (symbol.toLowerCase() === "tether" || symbol.toLowerCase() === "usd") {
      const usdBalance = wallet.balances.get("usd");
      const tetherBalance = wallet.balances.get("tether");
      const amountToDeduct = Math.min(usdBalance, amount);

      // Deduct from both balances
      wallet.balances.set("usd", usdBalance - amountToDeduct);
      wallet.balances.set("tether", tetherBalance - amountToDeduct);
    } else {
      // Deduct from the specific cryptocurrency balance
